...
-->
  ...
  <xsl:template match="tei:publicationStmt">
    ...
    <xsl:template name="recommendations">
        <!-- Tokenize rows -->
        <xsl:variable name="rows" select="tokenize($recs-tsv, '\n')" />
        ...
        <xsl:for-each select="$rows">
            <xsl:if test="starts-with(., xs:string($id))">
                ...
                <xsl:variable name="tabs" select="tokenize(., '\t')" />
                <!-- Set up the container for recommendations: -->
                    ...
                    <ol>
                        <li>
                            <!-- I'm was trying here to use a function I've created but it seems like I need a namespace to do that...
                            I am running the same code 5 times to re look over the rows to find the information about each of 
                            the recommendations. -->
                            <xsl:for-each select="$rows">
                                <xsl:if test="starts-with(., $tabs[8])">
                                    <!-- With each recommended article, we go back into the tsv to grab its own title, year, author
                                     ...
                                     <!-- Setting up the url and its <a> tag-->
                                        <xsl:element name="a">
                                            <xsl:attribute name="href">
                                                <xsl:value-of select="$tabs[18]"/>
                                            </xsl:attribute>
                                        <xsl:value-of select="$tabs[6]"/>
                                        <!-- selecting and inputting the year [2], author [4], and university [5] -->
                                        </xsl:element>,
                                        <xsl:value-of select="$tabs[2]"/>,
                                        <xsl:value-of select="$tabs[4]"/>, <xsl:value-of select="$tabs[5] "/>
                        ...
                        <li>
                            <xsl:for-each select="$rows">
                                <xsl:if test="starts-with(., $tabs[9])">
                                     <xsl:variable name="tabs" select="tokenize(., '\t')" />
                                        <xsl:element name="a">
                                            <xsl:attribute name="href">
                                                <xsl:value-of select="$tabs[18]"/>
                        ...
                        <li>
                            <xsl:for-each select="$rows">
                                <xsl:if test="starts-with(., $tabs[10])">
                                     <xsl:variable name="tabs" select="tokenize(., '\t')" />
                                        <xsl:element name="a">
                                            <xsl:attribute name="href">
                                                <xsl:value-of select="$tabs[18]"/>
                        ...
                        <li>
                            <xsl:for-each select="$rows">
                                <xsl:if test="starts-with(., $tabs[11])">
                                     <xsl:variable name="tabs" select="tokenize(., '\t')" />
                                        <xsl:element name="a">
                                            <xsl:attribute name="href">
                                                <xsl:value-of select="$tabs[18]"/>
                        ...
                        <li>
                            <xsl:for-each select="$rows">
                                <xsl:if test="starts-with(., $tabs[12])">
                                     <xsl:variable name="tabs" select="tokenize(., '\t')" />
                                        <xsl:element name="a">
                                            <xsl:attribute name="href">
                                                <xsl:value-of select="$tabs[18]"/>
        ...
        <!-- Tokenize rows -->
        <xsl:variable name="rows-kw" select="tokenize($recs-tsv-keywords, '\n')" />
        <!-- Find the row that matches this article's ID. -->
        <xsl:for-each select="$rows-kw">
            <xsl:if test="starts-with(., xs:string($id))">
                <!-- Tokenize that row, splitting by tab character. -->
                <xsl:variable name="tabs" select="tokenize(., '\t')" />
                <!-- Set up the container for recommendations: -->
