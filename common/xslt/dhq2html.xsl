  ...
  <xsl:template match="tei:publicationStmt">
    ...
    <xsl:template match="tei:quote[@rend='block']">
      <blockquote>
        ...
        <xsl:choose>
          <xsl:when test="   descendant::tei:list
                          or descendant::tei:sp
                          or descendant::tei:table
                          or descendant::tei:lg
                          or descendant::tei:xtext
                          or descendant::tei:p">
            <xsl:apply-templates select="@xml:lang|node()"/>
          </xsl:when>
          <xsl:when test="ancestor::tei:p">
            <p><xsl:apply-templates select="@xml:lang|node()"/></p>
          </xsl:when>
          <xsl:otherwise>
            <p class="ptext"><xsl:apply-templates select="@xml:lang|node()"/></p>
          </xsl:otherwise>
        </xsl:choose>
      </blockquote>
    </xsl:template>
      ...
      <xsl:choose>
          ...
          <blockquote class="quotedlist">
            ...
            <xsl:choose>
              <xsl:when test="child::tei:p">
                <!-- This should never happen: <tei:p> is not a valid
                     child of <tei:cit> —Syd, 2024-06-08 -->
                <xsl:apply-templates select="@xml:lang | tei:quote"/>
              </xsl:when>
              <xsl:when test="   descendant::tei:list
                              or descendant::tei:sp
                              or descendant::tei:table
              ...
              <xsl:otherwise>
                <!--
                    Might be better if @xml:lang were processed here,
                    (instead of inside <p class=ptext>, below) so that
                    output lang= went on <blockquote>, not <p>.
                    — Syd, 2024-06-08
                -->
                <p class="ptext"><xsl:apply-templates select="@xml:lang|node()"/></p>
              </xsl:otherwise>
        ...
        <xsl:when test="tei:quote[@rend='block']">
          <xsl:choose>
            ...
            <xsl:when test="descendant::tei:lg">
              <div class="lg"><xsl:apply-templates select="@xml:lang|node()"/></div>
            </xsl:when>
            <xsl:otherwise>
              <blockquote>
                <xsl:choose>
                  <xsl:when test="descendant::tei:list or descendant::tei:lg">
                    <xsl:apply-templates select="@xml:lang|node()"/>
                  </xsl:when>
    ...
    <xsl:template match="tei:epigraph">
      <xsl:choose>
        ...
        <xsl:when test="tei:quote[@rend='block']">
          <div>
            <xsl:apply-templates select="@xml:lange"/>
            <xsl:call-template name="assign-class"/>
            <xsl:choose>
    ...
    <xsl:template match="tei:p">
      ...
      <xsl:choose>
        <xsl:when test="parent::tei:note or ancestor::tei:list[@type='gloss']">
          <xsl:apply-templates/><br/><br/>
        </xsl:when>
        <xsl:when test="ancestor::dhq:abstract">
          <p><xsl:apply-templates select="@xml:lang|node()"/></p>
        </xsl:when>
        <xsl:when test="ancestor::tei:sp">
          <xsl:choose>
            <xsl:when test="   ancestor::tei:eg
                            or ancestor::tei:list
                            or ancestor::tei:quote[@rend='block']
                            or ancestor::tei:floatingText">
              <div class="ptext">
                  ...
                  <tr>
                    <xsl:choose>
                      <xsl:when test="preceding-sibling::tei:p">
                        <td class="speaker">&#160;</td><td><xsl:apply-templates/></td>
                      </xsl:when>
                      <xsl:otherwise>
        ...
        <xsl:when test="ancestor::tei:quote">
          <xsl:choose>
            <xsl:when test="(   parent::tei:quote/following-sibling::tei:ref
                             or parent::tei:quote/following-sibling::tei:ptr
                            )
                            and
                            following-sibling::tei:p">
              <p class="ptext"><xsl:apply-templates select="@xml:lang|node()"/></p>
            </xsl:when>
            <xsl:when test="( parent::tei:quote/following-sibling::tei:ptr )
                            and
                            not( child::tei:list )">
              <p class="quote">
                <xsl:apply-templates select="@xml:lang|node()"/>
                <xsl:apply-templates select="parent::tei:quote/following-sibling::tei:ptr"/>
              </p>
            </xsl:when>
            <xsl:when test="(parent::tei:quote/following-sibling::tei:ref)
                            and not(child::tei:list)">
            <p class="quote">
              <xsl:apply-templates select="@xml:lang|node()"/>
              <xsl:apply-templates select="parent::tei:quote/following-sibling::tei:ref"/>
            </p>
            </xsl:when>
            <xsl:otherwise>
              <div class="ptext"><xsl:apply-templates select="@xml:lang|node()"/></div>
            </xsl:otherwise>
          </xsl:choose>
        </xsl:when>
        <xsl:when test="   ancestor::tei:list
                        or ancestor::dhq:example
                        or ancestor::tei:eg
                        or ancestor::tei:figure">
          <div class="ptext"><xsl:apply-templates select="@xml:lang|node()"/></div>
        </xsl:when>
        <xsl:otherwise>
          <xsl:variable name="counter">
    ...
    <xsl:template match="tei:stage">
      <span class="stage">&#91;<xsl:apply-templates select="@xml:lang | node()"/>&#93;</span>
    </xsl:template>
    ...
    <xsl:template match="dhq:example | tei:figure">
      <div>
        <xsl:call-template name="id"/>
        <xsl:call-template name="assign-class"/>
        <xsl:apply-templates select="@xml:lang | node()"/>
    ...
    <xsl:template match="tei:gi">
      <!-- No application of @xml:lang, as contents of <gi> are not in
           a natural language. -->
    ...
    <xsl:template match="tei:att">
      <!-- No application of @xml:lang, as contents of <att> are not in
           a natural language. -->
      <span class="monospace">
        <xsl:value-of select="concat('@',normalize-space(.))"/>
      </span>
    ...
    <xsl:template match="tei:lb">
      <!-- No application of @xml:lang, as <lb> is empty -->
      <br/>
    </xsl:template>
    ...
    <xsl:template match="tei:figure/tei:head" mode="caption">
      ...
      <xsl:if test="child::node()">
        <div class="caption">
          <!-- If there is a language specified on the following
               <dhq:caption>, use that; if not use the one on this
               <head> (if any).  -->
          <xsl:apply-templates select="@xml:lang"/>
          <xsl:apply-templates select="following-sibling::dhq:caption/@xml:lang"/>
   ...
   <xsl:template match="tei:media">
     ...
     <xsl:choose>
       ...
       <xsl:otherwise>
         <div class="ptext" xml:lang="en" lang="en"><strong>Debug: </strong> Unhandled media type.</div>
       </xsl:otherwise>
     </xsl:choose>
   </xsl:template>

   <xsl:template match="tei:code">
      <!-- No application of @xml:lang, as contents of <code> are not in
           a natural language. -->
   ...
   <xsl:template match="tei:lg">
     <xsl:choose>
       <xsl:when test="ancestor::tei:cit or ancestor::tei:epigraph">
         <xsl:apply-templates select="@xml:lang | node()"/>
       </xsl:when>
       <xsl:otherwise>
         <div>
   ...
   <xdoc:doc>
    ...
    <xsl:template name="internal_ref">
      <xsl:choose>
        ...
        <xsl:otherwise>
          <span class="error">
            <xsl:apply-templates select="@xml:lang"/>
            <a href="{@target}"><xsl:apply-templates mode="scrubbing"/></a>
          </span>
        </xsl:otherwise>
      </xsl:choose>
    </xsl:template>
    ...
    <xsl:template match="tei:note" mode="notes_other">
      <xsl:variable name="language" select="ancestor-or-self::*[@xml:lang][1]/@xml:lang" as="xs:language"/>
      <div class="endnote">
        <xsl:apply-templates select="@xml:lang"/>
        <xsl:attribute name="id">
          <xsl:value-of select="generate-id()"/>
        ...
        <span>
          <xsl:attribute name="class">
            <xsl:value-of select="concat('noteRef lang ', $language )"/>
          </xsl:attribute>
          <xsl:text>[</xsl:text>
          <xsl:number level="any" from="//tei:body"/>
          <xsl:text>]&#160;</xsl:text>
    ...
    <xsl:template match="tei:ptr[@type = 'dhq-annex-embed']">
      <div lang="en" xml:lang="en">
        <iframe src="{@target}" style="{@style}">
          <p>Your browser does not support iframes. Embedded document may be found at <a href="{@target}"><xsl:value-of select="@target"/></a></p>
        </iframe>
        <p>
    ...
    <xsl:template name="bibliography">
      <xsl:if test="/tei:TEI/tei:text/tei:back/tei:listBibl">
        <div id="worksCited">
          <h2 lang="en" xml:lang="en">Works Cited</h2>
