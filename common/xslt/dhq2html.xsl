  ...
  <xsl:template match="tei:publicationStmt">
    ...
    <xsl:function name="dhqf:recs">
        ...
        <xsl:for-each select="$rows">
            ...
            <!-- With each recommended article, we go back into the tsv to grab its own title, year, author
                                     name, university, and url-->
    ...
    <xsl:template name="recommendations">
        ...
        <xsl:for-each select="$spector_rows">
            <xsl:if test="starts-with(., xs:string($id))">
                ...
                <xsl:variable name="spector_tabs" select="tokenize(., '\t')" />
                <!-- Set up the container for recommendations: -->
                    ...
                    <ol>
                        <li>
                            <!-- create variable to pass as parameter. This is index 6 of the tsv, or the first recommended article ID -->
                            <xsl:variable name="input_num" select="6" />
                            ...
                            <xsl:sequence select="dhqf:recs($input_num, $spector_tabs, $spector_rows)" />
                        </li>
                        <li>
                            <!-- We repeat for subsequent recommended article IDs 2 through 5 (index 7 through 10) -->
                            <xsl:variable name="input_num" select="7" />
                            <xsl:sequence select="dhqf:recs($input_num, $spector_tabs, $spector_rows)" />
                        </li>
                        <li>
                            <xsl:variable name="input_num" select="8" />
                            <xsl:sequence select="dhqf:recs($input_num, $spector_tabs, $spector_rows)" />
                        </li>
                        <li>
                            <xsl:variable name="input_num" select="9" />
                            <xsl:sequence select="dhqf:recs($input_num, $spector_tabs, $spector_rows)" />
                        </li>
                        <li>
                            <xsl:variable name="input_num" select="10" />
                            <xsl:sequence select="dhqf:recs($input_num, $spector_tabs, $spector_rows)" />
                        </li>
                    </ol>
            </xsl:if>
        ...
        <!-- Find the row that matches this article's ID. -->
        <xsl:for-each select="$keyword_rows">
            <xsl:if test="starts-with(., xs:string($id))">
                <!-- Tokenize that row, splitting by tab character. -->
                <xsl:variable name="keyword_tabs" select="tokenize(., '\t')" />
                <!-- Set up the container for recommendations: -->
                    ...
                    <ol>
                        <li>
                            ...
                            <!-- Calling the function to populate the recommendation. Pass in the input number, the current row of the selected Article,
                            and the whole set of rows in the tsv. -->
                            <xsl:sequence select="dhqf:recs($input_num, $keyword_tabs, $keyword_rows)" />
                        </li>
                        <li>
                            <!-- We repeat for subsequent recommended article IDs 2 through 5 (index 7 through 10) -->
                            <xsl:variable name="input_num" select="7" />
        ...
        <!-- Find the row that matches this article's ID. -->
        <xsl:for-each select="$tfidf_rows">
            <xsl:if test="starts-with(., xs:string($id))">
                <!-- Tokenize that row, splitting by tab character. -->
                <xsl:variable name="tfidf_tabs" select="tokenize(., '\t')" />
                <!-- Set up the container for recommendations: -->
                    ...
                    <ol>
                        <li>
                            ...
                            <!-- Calling the function to populate the recommendation. Pass in the input number, the current row of the selected Article,
                            and the whole set of rows in the tsv. -->
                            <xsl:sequence select="dhqf:recs($input_num, $tfidf_tabs, $tfidf_rows)" />
                        </li>
                        <li>
                            <!-- We repeat for subsequent recommended article IDs 2 through 5 (index 7 through 10) -->
                            <xsl:variable name="input_num" select="7" />
